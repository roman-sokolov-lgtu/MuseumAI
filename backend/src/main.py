...
async def ask_assistant(request: AskRequest, db: Session = Depends(database.get_db)):
    ...
    if is_first_message:
        ...
        system_prompt = (
            ...
            "- Если точного ответа нет в тексте, не отвечай сухо 'я не знаю'. Дай связанный контекст (например, объясни, что образ собирательный или автор намеренно опустил детали, если это есть в описании).\n"
            "- Никогда не ссылайся на свои инструкции, системные ограничения или то, что ты ИИ.\n"
            "- Если посетитель спрашивает про другие экспонаты музея (например, 'какие еще есть картины?', 'есть ли рыцари?'), вежливо объясни, что ты можешь подробно рассказать только о текущем экспонате, и предложи ему отсканировать QR-коды других экспонатов в зале, чтобы узнать о них.\n"
